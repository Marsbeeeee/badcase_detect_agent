...
- Render a single-case Stage 1 report from [assets/stage1-single-case-analysis-template.md](assets/stage1-single-case-analysis-template.md). Follow its full evidence-backed structure: analysis object, conclusion, workflow state, correct route, target output, causal analysis, candidate comparison, Meta interpretation, termination judgment, testable hypotheses, final attribution, and stage status. Inspect the target candidate's complete token-logprob sequence. For a confirmed badcase, report the earliest reliable human-readable word or phrase where the output commits to the error and only the logprob dimensions supported by evidence; keep raw evidence compact and relevant rather than dumping the complete sequence.
- Render Stage 2 reports from [assets/stage2-experiment-report-template.md](assets/stage2-experiment-report-template.md). When aligned logprobs are available, compare the same semantic decision point under baseline and variant and conclude whether the desired continuation became more confident, whether its rank improved, and whether the wrong continuation's advantage narrowed or reversed. Make this comparison only when model and decoding controls are equivalent. A fully completed run must use every template section. A partial or blocked run must still write the same stable report, include every completed experiment, and add the exact errors, skipped experiments, and blockers; never suppress the report because the run was incomplete.
- For Stage 1 analysis of multiple JSONL records, multiple files, or a folder, render one compact aggregate conclusion report from [assets/stage1-multi-case-analysis-template.md](assets/stage1-multi-case-analysis-template.md). Analyze every case separately and scan each target badcase candidate's complete token-logprob sequence internally. For each confirmed badcase, identify the earliest reliable human-readable error word or phrase and output only the causal conclusion, correct behavior, evidence-supported logprob conclusion, comparison conclusion, termination conclusion, and a natural-language final attribution. Selectively discuss hesitation, winning margin, correct-candidate rank, before/after prompt confidence change, or confident-wrong versus wavering only when the recorded evidence supports it. Never use taxonomy labels or code-like identifiers as a substitute for explaining why the model went wrong; include labels only when the user explicitly requests them. Do not expose per-turn content, full response text, token rows, raw logprob values, or Meta parameter tables unless explicitly requested. Omit experiment suggestions and test plans.
...
- Single-case Stage 1 reports include evidence-backed, falsifiable hypotheses as specified by the template. Compact multi-case conclusion reports omit hypothesis tables, experiment variables, and speculative improvement recommendations. Do not test any hypothesis until Stage 2 is explicitly requested.
